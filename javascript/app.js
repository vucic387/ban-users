jQuery(document).ready(function($) {
  ...
  $("table.wp-list-table.users, table.wp-list-table.comments, table.wp-list-table.posts").on(
    "click",
    ".js-w3dev-banned-history",
    function (e) {
      e.preventDefault();
  ...
  var _users_table = $("#the-comment-list, #the-list");
  if (_users_table.length > 0) {
    var rows = _users_table.find("> tr, > li");
    $.each(rows, function (index, value) {
      if ($(this).find("span.banned-user.row-highlight").length > 0) {
        $(this).addClass("w3dev-banned-user-row");
        $(this).find(".w3dev-banned-status").addClass("active row-highlight");
      }
    });
  }
      ...
      if (typeof _user_ban === "undefined") {
        ...
        if (_custom_email != 0) {
            ...
            onOpen: function () {
              ...
              $("#input-w3dev-ban-duration").on("change", function (e) {
                ...
                if (_option == "date picker") {
                  ...
                  if (_flatpickr) _flatpickr.close();
                }
              });
            },
            buttons: {
                ...
                action: function () {
                  ...
                  $.post(ajaxurl, data, function (response) {
                    ...
                    if (_accessibility) {
                      ...
                      /*  Added for post and comment rows, because there are more than one row per user */
                      if (_user_row.length == 0 && _user_id) {
                        $('a[data-user-id="' + _user_id + '"].toggle-ban-user').removeClass("active").addClass(_class);
                        $('a[data-user-id="' + _user_id + '"].warn-ban-user').addClass("hide");
                        _user_name = $('a[data-user-id="' + _user_id + '"].toggle-ban-user')
                          .closest("#the-comment-list .comment.byuser, #the-comment-list .banned-history.byuser, #the-list tr.type-banned-history")
                          .find(".author.column-author, .comment-author, .column-title.has-row-actions a.row-title")
                          .prop("innerText")
                          .split("\n")[0];
                        $("#the-comment-list .comment-author-" + _user_name).addClass("w3dev-banned-user-row");
                        $('a[data-user-id="' + _user_id + '"].toggle-ban-user').closest("#the-list tr.type-banned-history").addClass("w3dev-banned-user-row");
                        $('a[data-user-id="' + _user_id + '"].toggle-ban-user').closest("#the-list tr.type-banned-history").find(".w3dev-banned-status").attr("data-balloon", "Banned: " + _now).addClass("active row-highlight");
                        $("#the-comment-list .comment-author-" + _user_name).find(".w3dev-banned-status").attr("data-balloon", "Banned: " + _now).addClass("active row-highlight");
                      }
                    }
          ...
          $.post(ajaxurl, data, function (response) {
            ...
            _user_row.find(".warn-ban-user,.icon-ban-date-user").addClass("hide");
            _user_row
              .find(".w3dev-banned-status")
              .attr("data-balloon", "Banned: " + _now)
              .addClass("active row-highlight");
        ...
        $.post(ajaxurl, data, function (response) {
          ...
          if (_user_row.length == 0 && _user_id) {
            $('a[data-user-id="' + _user_id + '"].toggle-ban-user').removeClass("active").addClass(_class);
            $('a[data-user-id="' + _user_id + '"].warn-ban-user').removeClass("hide");
            _user_name = $('a[data-user-id="' + _user_id + '"].toggle-ban-user')
              .closest("#the-comment-list .comment.byuser, #the-comment-list .banned-history.byuser, #the-list tr.type-banned-history")
              .find(".author.column-author, .comment-author, .column-title.has-row-actions a.row-title")
              .prop("innerText")
              .split("\n")[0];
            $("#the-comment-list .comment-author-" + _user_name).removeClass("w3dev-banned-user-row");
            $('a[data-user-id="' + _user_id + '"].toggle-ban-user').closest("#the-list tr.type-banned-history").removeClass("w3dev-banned-user-row");
            $('a[data-user-id="' + _user_id + '"].toggle-ban-user').closest("#the-list tr.type-banned-history").find(".w3dev-banned-status").attr("data-balloon", "Banned: " + _now).removeClass("active row-highlight");
            $("#the-comment-list .comment-author-" + _user_name).find(".w3dev-banned-status").attr("data-balloon", "Reinstated: " + _now).removeClass("active row-highlight");
          }
  ...
  $("#the-comment-list, #the-list").on("click", ".warn-ban-user", function (e) {
    var _this = $(this);
    var _user_id = _this.data("user-id");
    ...
    if (_reason != 0) {
      var jc = $.confirm({
        ...
        content:
          '<div id="jconfirm-warn-user"><p>Enter a brief message below to send to the user that explains the reason for the warning.</p><textarea name="" >' +
          php_vars.default_warn_reason +
          "</textarea></div>",
        useBootstrap: false,
        boxWidth: "400px",
        buttons: {
          ...
          send: {
            ...
            action: function () {
              ...
              _this_btn.prev("button").remove();
              _this_btn.html(
                '<i class="fa fa-spinner fa-spin fa-fw" aria-hidden="true"></i> SENDING'
              );
              ...
              $.post(ajaxurl, data, function (response) {
                ...
                if (!_accessibility) {
                  _this
                    .find(".fa-stack-1x")
                    .removeClass("fa-spinner fa-spin fa-fw")
  ...
  $("#input-w3dev-ban-user").on("change", function (e) {
    ...
    } else $("#w3dev-ban-reason").val("");

    if (_this.is(":checked")) $("#js-w3dev-unban-date").show();
    else $("#js-w3dev-unban-date").hide();
  ...
  $("#input-frontend-notification-force-logout").on("change", function (e) {
    var _this = $(this);
    if (_this.is(":checked")) {
      $("#input-frontend-notification-hide")
        .prop("checked", false)
        .attr("disabled", true);
    } else {
  ...
  $("#w3dev-save-ban-user-settings").on("click", function (e) {
    ...
    var _on_ban_change_user_role = $("#input-on-ban-change-user-role").is(
      ":checked"
    )
      ? 1
      : 0;
    var _set_banned_user_role = $("#input-set-banned-user-role").val();
    var _on_unban_change_user_role = $("#input-on-unban-change-user-role").is(
      ":checked"
    )
      ? 1
      : 0;
    var _set_unbanned_user_role = $("#input-set-unbanned-user-role").val();
    var _enable_support_cpt = $("#input-enable-support-cpt").is(":checked")
      ? 1
      : 0;
    ...
    var _ban_email_default = $("#input-ban-email-default").is(":checked")
      ? 1
      : 0;
    var _ban_email_message = $("#input-ban-email-message").val();
    var _hide_banned_users_comments = $("#input-hide-banned-users-comments").is(
      ":checked"
    )
      ? 1
      : 0;
    var _scramble_banned_users_password = $(
      "#input-scramble-banned-users-password"
    ).is(":checked")
      ? 1
      : 0;
    var _disable_password_reset_banned_users = $(
      "#input-disable-password-reset-banned-users"
    ).is(":checked")
      ? 1
      : 0;
    var _set_spammer_option = $("#input-set-spammer-option").is(":checked")
      ? 1
      : 0;
    var _unset_spammer_option = $("#input-unset-spammer-option").is(":checked")
      ? 1
      : 0;
    var _default_ban_reason = $("#input-default-ban-reason").val();
    var _default_warn_reason = $("#input-default-warn-reason").val();

    var _users_tbl_row_highlighted = $("#input-users-tbl-row-highlighted").is(
      ":checked"
    )
      ? 1
      : 0;
    var _users_tbl_data_column = $("#input-users-tbl-data-column").is(
      ":checked"
    )
      ? 1
      : 0;
    ...
    var _send_notification_new_post = $("#input-send-notification-new-post").is(
      ":checked"
    )
      ? 1
      : 0;
    var _notification_emails = $("#input-notification-emails").val();
    var _users_tbl_geoip_data_column = $(
      "#input-users-tbl-geoip-data-column"
    ).is(":checked")
      ? 1
      : 0;
    var _capture_login_geoip_data = $("#input-capture-login-geoip-data").is(
      ":checked"
    )
      ? 1
      : 0;
    ...
    var _frontend_banned_notification = $(
      "#input-frontend-banned-notification"
    ).is(":checked")
      ? 1
      : 0;
    var _frontend_notification_force_logout = $(
      "#input-frontend-notification-force-logout"
    ).is(":checked")
      ? 1
      : 0;
    var _frontend_notification_hide = $("#input-frontend-notification-hide").is(
      ":checked"
    )
      ? 1
      : 0;
    var _enable_accessibility = $("#input-enable-accessibility").is(":checked")
      ? 1
      : 0;
    var _security_enable_admin_override = $(
      "input[name=input-enable-admin-override]"
    ).is(":checked")
      ? 1
      : 0;
    var _security_set_moderator_roles = $(
      "input[name=input-security-set-moderator-roles]"
    ).is(":checked")
      ? 1
      : 0;
    var _security_moderator_roles = $(
      "select[name=input-security-moderator-roles]"
    ).val();
    var _security_set_moderated_roles = $(
      "input[name=input-security-set-moderated-roles]"
    ).is(":checked")
      ? 1
      : 0;
    var _security_moderated_roles = $(
      "select[name=input-security-moderated-roles]"
    ).val();

    // extensions
    // --
    var _ext_ultimate_member = $("#input-ext-ultimate-member").is(":checked")
      ? 1
      : 0;
    
    var _ext_woocommerce = $("#input-ext-woocommerce").is(":checked")
    ? 1
    : 0;
    ...
    var _autoload_jq_confirm = $("#input-autoload-jq-confirm").is(":checked")
      ? 1
      : 0;
    var _autoload_datatables = $("#input-autoload-datatables").is(":checked")
      ? 1
      : 0;
    var _autoload_notify = $("#input-autoload-notify").is(":checked") ? 1 : 0;
    var _autoload_selectric = $("#input-autoload-selectric").is(":checked")
      ? 1
      : 0;
    var _autoload_flatpickr = $("#input-autoload-flatpickr").is(":checked")
      ? 1
      : 0;
    var _autoload_alertify = $("#input-autoload-alertify").is(":checked")
      ? 1
      : 0;
    var _autoload_faanimation = $("#input-autoload-faanimation").is(":checked")
      ? 1
      : 0;
    ...
    var data = {
      ...
      ext_woocommerce: _ext_woocommerce,
  ...
  $("#w3dev-save-ban-email-template").on("click", function (e) {
    ...
    var _w3dev_unban_indefinite_date_tag = $(
      "#input-unban-indefinite-date-tag"
    ).val();
  ...
  // Highlight comment rows on the CPT banned-history for banned users
  var target = $("table.wp-list-table.comments-box #the-comment-list");
  ...
  var observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      ...
      if (newNodes !== null) {
        ...
        $nodes.each(function () {
          var $node = $(this);
          if ($node.hasClass("comment") || $node.hasClass("banned-history")) {
            // workarround for wp-admin/js/post.js ajax load comments
            // which gets number off loaded comments (st) by selector $('#the-comment-list tr.comment:visible').length
            // so, it doesn't work for custom type comments, only with default type 'comment'
            if (!$node.hasClass("comment"))  $node.addClass("comment");
              // do something
              if ($node.find("span.banned-user.row-highlight").length > 0) {
                $node.addClass("w3dev-banned-user-row");
                $node.find(".w3dev-banned-status").addClass("active row-highlight");
              }        
          }
        });
      }
...
if (target.length > 0) observer.observe(target[0], config);

// Later, you can stop observing
// Allways monitor because off ajax loading more comments, don not disconnect observer
//   observer.disconnect();
