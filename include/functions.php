...
{
    /* Moved out from wp settings (add_options_page), Settings main menu */
    add_menu_page(
        'BAN Users',
        'BAN Users',
        'manage_options',
    ...
    add_submenu_page(
        ...
        'Banned History',
        'manage_options',
        'edit.php?post_type=banned-history'
    );
}
...
function w3dev_check_if_banned_user()
{
    ...
    // Return error if user account is banned
    // Added isset($user->ID) for v1.5.4
    // Error happens rarely, when someone clears the history (cookies)
    if (isset($user->ID) && get_user_option('w3dev_user_banned', $user->ID, FALSE)) {
        if (!empty($settings['display_message'])) {
            ...
            $logout_url = (!empty($settings['custom_logout'])) ? $settings['custom_logout_url'] : wp_login_url();
            ...
            if (!is_admin()) {
                ...
                if (!empty($settings['display_message']) && !empty($settings['frontend_banned_notification'])) {
                    ...
                    if ($show_dialog) {
                        ...
                        {
                            global $current_user; // added v1.5.4
                            $w3dev_ban_user_class   = W3DEV_BAN_USER_CLASS::get_instance();
                            $settings               = $w3dev_ban_user_class->get_options('settings');
